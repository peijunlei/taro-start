...
export default class GiftMask extends Component<Partial<IGiftMaskProps>, T.IGiftMaskState> {
  ...
  _giftListInit = (level, giftGoodsInfos, selectedMarketingGifts, marketingSubType, stores, initGiftMarketLevelId) => {
    return level.fullGiftDetailList.map((detail, key) => {
      ...
      const {tradeItems, tradeConfirmMarketingList} = stores[0];
      const giftLevelId = detail.giftLevelId;
      const skuIds = tradeConfirmMarketingList.filter((item) => item.marketingLevelId === giftLevelId)?.[0]?.skuIds;
      const newTradeItems = tradeItems.filter((item) => skuIds.includes(item.skuId));
      //2021-11-02 禁用相关已转移后端处理
      let disabled = goodsInfo && goodsInfo.goodsStatus === 0 && goodsInfo.stock > 0;

      //满赠&&领取赠品 未达到级别 禁用
      if (marketingSubType === 5) {
        const fullCount = newTradeItems.map((item) => item.num).reduce((a, b) => _.add(a, b));
        // const fullCount = newTradeItems[0].num;
        disabled = disabled && fullCount < level.fullCount;
      }

      if (marketingSubType === 4) {
        const fullAmount = newTradeItems.map((item) => _.mul(item.num, item.price)).reduce((a, b) => _.add(a, b));
        // const fullAmount = newTradeItems[0].num * newTradeItems[0].price;
