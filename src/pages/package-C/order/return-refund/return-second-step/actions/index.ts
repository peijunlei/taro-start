    ...
    async init(tid) {
      ...
      if (isReturn) {
        ...
        tradeDetail['tradeItems'].forEach((v) => {
          ...
          v.price = _.addZeroFloor(v.splitPrice / v.num); //初始化每个商品的均摊平均价格
          v.skuPoint = _.addZeroFloor((v.points || 0) / v.num); //初始化每个商品的均摊平均积分(向下截取小数点后两位)
          v.num = v.canReturnNum; //初始化默认的退货数量
        });
      } else {
        //退款申请，获取退款原因列表
        returnReasonList = (await returnOrderBaseController.findReturnReason()) || [];
        returnReasonList = (returnReasonList['refundCauseVOList'] as any).map((v) => {
          for (let k in v) {
            return {
              id: v.id,
              name: v['cause'],
        ...
        hasReturnPrice =
          hasReturnPrice + returnOrderListRes.reduce((price, order) => price + order.returnPrice.applyPrice, 0) || 0;
        hasReturnPoints =
          hasReturnPoints + returnOrderListRes.reduce((point, order) => point + order.returnPoints.applyPoints, 0) || 0;
      }
      dispatch({
        type: Command.init,
        payload: {
      ...
      dispatch({
        ...
        payload: {
          main: {
            ...
            returnGoodsStateList: [
              {id: '0', name: '我未收到货'},
              {id: '1', name: '我已收到货'},
            ],
            returnType: res[2]['returnType'],
            selectReturnState: '0',
            selectedReturnWay: '1'
          },
        },
