...
export default (dispatch: Dispatch) => {
  let action = {
    ...
    changeIsShowState(type, value) {
      action.commonChange({paths: `main${type}`, value: value});
    },
    ...
    */
    ...
    async applyReturns() {
      ...
      if (
        returnType === 'REFUND' &&
        !FormRegexUtil(selectReturnState, '货物状态', {
          required: true,
        })
      ) {
        return;
      ...
      // 货物状态
      let goodsInfoState = selectReturnState === '0' ? 'NOT_RECEIVED' : 'RECEIVED';
      if(returnType === "REFUND") {
        param = param.set('goodsInfoState', goodsInfoState);
      }


      if (
        !FormRegexUtil(selectedReturnReason, '退款原因', {
          required: true,
        })
      ) {
        return;
      ...
      let checkList = returnReasonList.filter((item) => {
        return item.id === selectedReturnReason;
      });
      param = param.set('refundCause', {
        id: checkList[0]['id'],
        cause: checkList[0]['name'],
        sort: checkList[0]['sort'],
      });
      // 退单附件
      param = param.set(
        'images',
