...
import {isEmpty, debounce, floor} from 'lodash';
...
export default (dispatch: Dispatch) => {
  let action = {
    ...
    isRefund() {
      const {
        main: {flowState,payState,crossBorderFlag},
      } = getData();
      if(crossBorderFlag) {
        const applyRefundButtons = [['AUDIT', 'PAID']];
        if(applyRefundButtons.filter((button) => this._calc(button)(flowState, payState)).length > 0) {
          return true
        }
      } else {
        return false
            ...
            if (4 == giftMarketing.get('subType')) {
              const leftSkuAmount = giftMarketing
                ...
                .map((skuId) => {
                  ...
                  return skuItem.get('levelPrice') * (skuItem.get('num') - comReSkuCount - currReSkuCount); //某商品的发货商品价格 - 已退商品价格 - 当前准备退的商品价格
                })
              ...
              const leftSkuCount = giftMarketing
                ...
                .map((skuId) => {
                  ...
                  const indexTmp = currReturnSkus.findIndex((item) => item.get('skuId') == skuId);
                  const currReSkuCount = indexTmp > -1 ? currReturnSkus.get(indexTmp).get('num') : 0;
                  return skuItem.get('skuBuyNum') - comReSkuCount - currReSkuCount; //某商品的发货商品数 - 已退商品数 - 当前准备退的商品数
                })
    ...
    */
     ...
     * @param cb
     */
    async returnSkuSecond(returnType) {
      const {
        main: {tid, skus, gifts, newPoints, newPrice, isLoading, },
      } = getData();
      if(isLoading) return;
      ...
      param = param.set('returnPrice', {
        totalPrice: floor(newPrice, 2),
      });
      param = param.set('returnPoints', {
        applyPoints: Math.floor(newPoints),
      });
      param = param.set('returnGift', returnGift);
      param = param.set('returnType', returnType);

      await api.returnOrderController.transfer(param.toJS()).finally(() => {
        setTimeout(() => {
          action.commonChange('main.isLoading',false)
        }, 1000);
      });
          ...
          skus
            ...
            .map((sku) => {
              if (sku.num < sku.canReturnNum) {
                //小于可退数量,直接单价乘以数量
                return _.mul(floor(sku.price, 2), sku.num);
              } else {
        ...
        let points =
          skus
            ...
            .map((sku) => {
              if (sku.num < sku.canReturnNum) {
                ///小于可退数量,直接单价乘以数量
                let point = _.mul(sku.skuPoint, sku.num);
                return _.parseNumber(point);
              } else {
                //大于等于可退数量 , 使用分摊小计金额 - 已退金额(单价*(购买数量-可退数量))
                return sku.canReturnPoint;
              }
            })
            .reduce((one, two) => _.add(one, two), 0) || 0;
        points =  _.parseNumber(points);
        action.commonChange([
