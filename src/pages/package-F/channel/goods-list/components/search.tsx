...
export default class Search extends Component<Partial<ISearchProps>, T.ISearchState> {
  ...
  research = () => {
    let {
      ifStore,
      request: { storeId, keywords,goodsChannelActivityId},
      spreadFlag,
      isTabbar
    } = this.props.main;
    ...
    if(isTabbar){
      Taro.navigateTo({
        url: ifStore
          ...
          : spreadFlag
            ? `/pages/package-F/channel/goods-search/index?key=distributeGoods&channelId=${goodsChannelActivityId}`
            : `/pages/package-F/channel/goods-search/index?keywords=${keywords || ''}&channelId=${goodsChannelActivityId}&isTabbar=${isTabbar}`,
      });
    }else{
      Taro.redirectTo({
        url: ifStore
          ...
          : spreadFlag
            ? `/pages/package-F/channel/goods-search/index?key=distributeGoods&channelId=${goodsChannelActivityId}`
            : `/pages/package-F/channel/goods-search/index?keywords=${keywords || ''}&channelId=${goodsChannelActivityId}`,
      });
    }
  ...
  researchDel = (e) => {
    ...
    const {
      ifStore,
      request: { storeId,goodsChannelActivityId },
      spreadFlag,
    } = this.props.main;
    ...
    Taro.redirectTo({
      url: ifStore
        ...
        : spreadFlag
          ? `/pages/package-F/channel/goods-search/index?key=distributeGoods&channelId=${goodsChannelActivityId}`
          : `/pages/package-F/channel/goods-search/index?keywords=&channelId=${goodsChannelActivityId}`,
    });
  };
}
