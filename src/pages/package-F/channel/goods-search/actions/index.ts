...
import packageFGoodsSearchMain from '../reducers/main';
...
export default (dispatch: Dispatch) => {
  const actions = {
     ...
     * 初始化数据
     */
    async init(key, keywords,channelId,isTabbar) {
      await actions.loadReducer();
      await actions.action.commonChange('main.isLoadingFlag', true);
      actions.action.commonChange('main.channelId', channelId);
      actions.action.commonChange('main.isTabbar', isTabbar?true:false);
      if (key) {
    ...
    async search(relatedLandingPage?) {
      let {key = '', keywords = '', preKeywords = '',channelId} = getData().main;
        ...
        if (key === 'goods') {
          if (skuId) {
            ...
            if (!keywords && preKeywords) {
              url = `/pages/package-F/channel/goods-list/index?keywords=${preKeywords}&channelId=${channelId}`;
            } else {
              url = `/pages/package-F/channel/goods-list/index?keywords=${keywords}&channelId=${channelId}`;
            }
          }
        } else if (key === 'supplier') {
          ...
          url = `/pages/package-F/channel/goods-list/index?spreadFlag=${true}&keywords=${keywords}&channelId=${channelId}`;
    ...
    async unloadReducer() {
      if (reduxStore.deregister) {
        reduxStore.deregister(['packageFGoodsSearchMain']);
      } else {
