...
export default class packageFGoodsSearch extends Component<Partial<T.IProps>, any> {
  componentWillMount() {
    if (__TARO_ENV !== 'h5') {
      Taro.showShareMenu({
  ...
  componentDidShow() {
    ...
    const {key, keywords,channelId,isTabbar} = getCurrentInstance().router.params;
    this.props.actions.init(key, keywords,channelId,isTabbar);
  }
  ...
  render() {
    ...
    return (
      main &&
      main.key && (
        <View className="packageFGoodsSearch">
          <SearchBar />
          {/*分销员搜索不展示此标签*/}
          {WMkit.isMall() ||
          main.key == 'distribute' ||
