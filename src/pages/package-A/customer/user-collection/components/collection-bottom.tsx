...
import WMCheckbox from './collection-checkbox';
...
*/
  render() {
    let {
      ...
      main: {selectedList, ifSelectAll, list, ifModify, reload},
    } = this.props;
    const invalidGoods = list.filter((item) => item.goodsStatus === 2);
    return (
      <View className={isIphoneX ? 'collectionBottom bottomX' : 'collectionBottom'}>
        <WMCheckbox
          label="全选"
          checked={ifSelectAll}
          onClick={() => {
