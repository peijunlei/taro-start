...
import packageAGoodsAllListMainGoodsList from '../reducers/main';
  ...
  const actions = {
    ...
    async loadReducer() {
      reduxStore.registerReducer({
        packageAGoodsAllListMainGoodsList,
      });
    },
    ...
    async unloadReducer() {
      if (reduxStore.deregister) {
        reduxStore.deregister(['packageAGoodsAllListMainGoodsList']);
      } else {
...
function getData(): IAllReducerProps {
  return {
    main: reduxStore.getReducerData('packageAGoodsAllListMainGoodsList'),
  };
}
