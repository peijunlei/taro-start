  ...
  let action = {
    ...
    async query(isResetPage: boolean = false, goodsShowType, type?) {
      ...
      try {
        if (token && !spreadFlag) {
          //已登录 0:sku列表,1:spu列表
          result = goodsShowType == 0 ? await this.findSkuGoods() : await this.findSpuGoods();
        } else if (spreadFlag) {
          ...
          //未登录 0:sku列表,1:spu列表
          result = goodsShowType === 0 ?
            await this.noLoginSkuFindGoods()
            : await this.noLoginSpuFindGoods()
        }
      } catch (e) {
      ...
      // console.log('goods2',getData().main,goods)
      if (pageNum === 0 && !isReady) {
      ...
      // if (loadSkeleton) {
      //   action.commonChange([{paths: 'main.loadSkeleton', value: false}]);
      // }
      // action.commonChange('main.loadSkeleton', false);
      ...
      //   }, 500);
      // } else {
      //   action.commonChange('main.loadStatus', 'c');
      // }
      // action.commonChange('main.loadStatus', 'loaded');
    ...
    _dealRequestParam(param) {
      ...
      const requestParam = { cateAggFlag: true, areaId: chooseCity.addrId };
