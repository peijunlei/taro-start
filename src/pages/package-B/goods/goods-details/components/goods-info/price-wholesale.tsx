...
*/
  render() {
    ...
    //划线价
    const lineShowPrice = null;
    //最低,最高价格
    const {minPrice, maxPrice, minPoint, maxPoint} = this._getMinMaxPrice(goodsDetail.goods, goodsDetail.goodsInfos);
        ...
        {priceType == 1 && allowPriceSet == 0 ? (
          <ScrollView scrollX className="scroll-view-box">
            <View className="no-price-box">
              {this._showSpuIntervalPrices().map((item, index) => {
                return (
                  <View className="no-goods-wholeSale-price" id={index} key={index}>
          ...
          <View className="price">
            ...
            <Text className="now-price">
              ...
              {minPrice == maxPrice
                ? toFixed2(minPrice ? minPrice : 0)
                : toFixed2(minPrice ? minPrice : 0) + '~' + toFixed2(maxPrice)}
            </Text>

            {lineShowPrice && <Text className="old-price">￥{lineShowPrice}</Text>}
          </View>
        )}
      </View>
    );
  ...
  _getMinMaxPrice = (goods, goodsInfos) => {
    ...
    if (goods.priceType == 1) {
      ...
      goodsInfos.forEach((info, index) => {
        if (index == 0) {
          ...
          minPoint = info.buyPoint < minPoint ? info.buyPoint : minPoint;
    ...
    return {minPrice, maxPrice, minPoint, maxPoint};
  };
  /**
   * 不允许独立设价 展示阶梯价格
