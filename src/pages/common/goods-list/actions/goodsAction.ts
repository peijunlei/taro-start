...
import api from 'api';
//更新购物车角标
import {setShopCartNum, getShopCartNum, WMkit} from 'wmkit';
//加入购物车的商品数量
let cartNum = 0;
export default (dispatch: Dispatch) => {
  let action = {
    ...
    async nextPage(force?: boolean) {
      ...
      // 第一次查询查 goodsShowType
      if (request.pageNum === 0) {
        let {goodsShowType} = await this.goodsShowType();
      }
      //商品列表查询
      await dispatch({type: Command.modifyRequest, payload: {pageNum: request.pageNum + 1}});

      await this.query(false, goodsShowType);
      await action.commonChange([{paths: 'main.loadStatus', value: 'loaded'}]);
    },

    /**
     * 以当前查询条件查询
     */
    async query(isResetPage: boolean = false, goodsShowType) {
      let {
        spreadFlag,
        request,
        goods,
      ...
      await action.commonChange('main.loadSkeleton', true);
      ...
      if (token && !spreadFlag) {
        ...
        result = await goodsInfoBaseController.addDistributorGoods(getData().main.request);
      } else {
        //未登录 0:sku列表,1:spu列表
      ...
      if (request.pageNum === 0 && !isReady) {
        ...
        action.commonChange([
          ...
          {paths: 'main.totalPages', value: showFlag ? totalPages : esTotalPages},
    ...
    async noLoginSpuFindGoods() {
      ...
      let retult = await goodsBaseController.spuListFront(await this._dealRequestParam(request));
      return retult;
    },
    ...
    async noLoginSkuFindGoods() {
      let {request} = getData().main as any;
