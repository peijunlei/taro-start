...
const controllerName = 'GoodsExcellentRecommendController';
 ...
 *
 */
async function getExcellentProduct(): Promise<GoodsExcellentResponse> {
  let result = await sdk.get<GoodsExcellentResponse>(
    '/goods/excellent/get-Recommend',
 ...
 * via the `definition` "BaseResponse«GoodsExcellentResponse»".
 */
export interface BaseResponseGoodsExcellentResponse {
  /**
   * 结果码
   */
  code: string;
  context?: GoodsExcellentResponse;
  /**
   * 消息内容
   */
  message?: string;
 ...
 * 内容
 */
export interface GoodsExcellentResponse {
  /**
   * 商品列表
   */
...
/**
 * This interface was referenced by `IgnoreType`'s JSON-Schema
 * via the `definition` "GoodsExcellentResponse".
 */
export interface GoodsExcellentResponse1 {
  /**
   * 商品列表
   */
  goodsInfos?: GoodsInfoVO[];
