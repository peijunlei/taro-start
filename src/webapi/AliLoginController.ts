...
const controllerName = 'AliLoginController';
 ...
 */
async function bind(
  aliBindRequest: IBindAliBindRequestReq,
): Promise<LoginResponse> {
  let result = await sdk.post<LoginResponse>(
    '/third/login/ali/bind',
 ...
 * via the `definition` "AliBindRequest".
 */
export interface AliBindRequest {
  /**
   * Ali登录accessToken
   */
  accessToken?: string;
...
/**
 * This interface was referenced by `IgnoreType`'s JSON-Schema
 * via the `definition` "IBindAliBindRequestReq".
 */
export interface IBindAliBindRequestReq {
  /**
   * Ali登录accessToken
   */
  accessToken?: string;
